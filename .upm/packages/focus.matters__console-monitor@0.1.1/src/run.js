...
import { DEFAULT_HOST, DEFAULT_PORT_FILE_NAME } from "./tcp-client.js";
...
export function printRunUsage() {
  console.error(
    [
      ...
      `  - The active TCP port is written to <cwd>/${DEFAULT_PORT_FILE_NAME} unless --port-file is set.`,
...
function parseArgs(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (parsingFlags && token.startsWith("--port-file=")) {
      options.portFile = resolve(token.slice("--port-file=".length));
      continue;
    }

    if (parsingFlags && token === "--port-file") {
      index += 1;
      if (index >= argv.length) {
        throw new Error("Missing value for --port-file");
      }
      options.portFile = resolve(argv[index]);
      continue;
    }
...
export function runConsoleMonitor(argv = process.argv.slice(2)) {
  let parsed;
  ...
  const currentPortFile = parsed.options.portFile || resolve(parsed.options.cwd, DEFAULT_PORT_FILE_NAME);
  ...
  function writeCurrentPortFile(port) {
    writeFileSync(currentPortFile, `${port}\n`, "utf8");
  ...
  function clearCurrentPortFile() {
    ...
    try {
      const currentPort = readFileSync(currentPortFile, "utf8").trim();
      if (currentPort === String(activePort)) {
        unlinkSync(currentPortFile);
      }
    } catch (error) {
      if (error?.code !== "ENOENT") {
        console.error(`[cm run] Failed to clear ${currentPortFile}: ${error.message}`);
      }
    } finally {
  ...
  function handleTcpRequest(socket, rawRequest) {
    ...
    if (request.type === "restart") {
      ...
      writeSocketText(socket, `[cm run] ${message}\n`);
    }
  }
      ...
      socket.on("data", (chunk) => {
        ...
        while (newlineIndex >= 0) {
          ...
          try {
            ...
            writeSocketText(socket, `[cm run] ${error.message}\n`);
          }
  ...
  function handleServerError(error) {
    ...
    console.error(`[cm run] TCP server error on ${host}:${port}: ${error.message}`);
      ...
      function attemptListen(port) {
        ...
        const handleInitialError = (error) => {
          if (error?.code === "EADDRINUSE") {
            console.error(`[cm run] TCP port ${port} on ${configuredHost} is already in use, trying ${port + 1}`);
  ...
  listenTcpServer(configuredPort)
    .then((listeningServer) => {
      ...
      if (address && typeof address === "object") {
        ...
        console.error(`[cm run] TCP listening on ${address.address}:${address.port}${configuredLabel}`);
        console.error(`[cm run] Port file: ${currentPortFile}`);
      }
    })
    .catch((error) => {
